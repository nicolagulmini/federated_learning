...
def plot_cluster_performances(clusters, iterations, percentage):
    for cluster in clusters:
        ...
        for i in range(iterations):
            tmp_model = tf.keras.models.load_model("classification"+str(percentage)+"/cluster"+str(cluster.number)+"_"+str(percentage)+"sparse_iter"+str(i)+".h5")
            _, accuracy = tmp_model.evaluate(X_test, Y_test)
            cl_acc.append(accuracy)
        plt.plot(range(iterations), cl_acc)
    # sistema

def plot_average_cluster_performances(clusters, iterations, percentage):
    ...
    plt.ylim((0, 80))
    name = str(percentage) + "-sparse_local_CNN.png"
    plt.savefig(name)
    
def test_aggregation(X_test, Y_test, clusters, percent_classification, percent_estimation, cluster_iteration, est_iteration):
    true_predictions = 0
    for t in range(len(X_test)):
        ...
        log_probs = []
        for cluster in clusters:
            tmp_model = tf.keras.models.load_model("./classification"+str(percent_classification)+"/cluster"+str(cluster.number)+"_"+str(percent_classification)+"sparse_iter"+str(cluster_iteration)+".h5")
            tmp_est = NICE(data_dim=3072, num_coupling_layers=3)
            tmp_est.load_state_dict(torch.load("./estimation"+str(percent_estimation)+"/"+"estimation_cluster"+str(cluster.number)+"sparse_"+str(percent_estimation)+"_iter"+str(est_iteration)+".pt"))
